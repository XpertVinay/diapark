                ...
                <div class="row">
                    ...
                    <div class="col-xl-2 col-lg-3 col-md-4 fixed-top sidebar bg-dark text-light p-0">
                        <a href="{{url('/dashboard')}}" class="navbar-brand text-light text-uppercase d-block mx-auto mb-3 button-border p-2">
                            <h5 class="text-center"><i class="fas fa-utensils"></i> SOW </h5>
                            <p class="text-center">Some Tag Lines</p>
                        </a>
                        ...
                        <ul class="navbar-nav flex-column">
                            ...
                            @endif
                            <li class="nav-item">
                                <a href="{{auth()->user() ? url('/tablereservation') : url('/staff/tablereservation')}}" class="nav-link text-white mb-2 sidebar-link">
                                    <i class="fas fa-table fa-lg mr-2"></i>Reservations
                                </a>
                            </li>
                            @if(Auth::user())
                            <li class="nav-item">
                                <a href="{{ url('/restaurants') }}" class="nav-link text-white mb-2 sidebar-link">
                                    <i class="fas fa-table fa-lg mr-2"></i>Restaurants
                                </a>
                            </li>
                            @endif
                            @if(false)
                    ...
                    <ul class="nav">
                        ...
                        <li class="nav-item ml-auto">
                            <a class="nav-link dropdown-toggle" href="#" id="navbarDropdown" role="button" data-toggle="dropdown" style="font-size: 1rem; text-transform: lowercase;">
                                <i class="fas fa-user-circle fa-lg"></i> {{ Auth::user() ? Auth::user()->name : $request->session()->get('name') }}
                            </a>
    ...
    <div class="modal fade" id="editProfileModal">
        <div class="modal-dialog modal-md">
            <div class="modal-content">
                ...
                <form action="@if(Auth::user()){{route('updateprofile.submit')}}@else{{route('updatestaffprofile.submit')}}@endif" method="post">
                @csrf
                    <div class="modal-body">
                        <div class="form-group">
                            <label for="roomtype">Name</label>
                            <input type="text" class="form-control" name="name" placeholder="Enter Full Name" value="{{Auth::user() ? Auth::user()->name : $request->session()->get('name')}}" required="true">
                        </div>
                        <div class="form-group">
                            <label for="email">Email</label>
                            <input type="email" class="form-control" name="email" placeholder="Enter Your Email" value="{{Auth::user() ? Auth::user()->email : $request->session()->get('email')}}" required="true">
                        </div>
                        <div class="form-group">
