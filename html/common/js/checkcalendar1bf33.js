...
////Datepickerの日付がクリックされた時にプルダウンを変更させる関数
	function setDatePulldown1(date) {
		...
		//トップページ
		if($("#dk0-start_month").size()){
			$("#dk0-combobox").text(dates[0]+'年 '+dates[1]+'月');
			$("#dk1-combobox").text(dates[2]+'日');
		}
		...
		if(date1 >= date2){
			...
			$("#end_day").val(dates[2]);
			if($("#dk2-end_month").size()){
				$("#dk2-combobox").text(dates[0]+'年 '+dates[1]+'月');
				$("#dk3-combobox").text(dates[2]+'日');
			}
		}
